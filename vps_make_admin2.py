...
# Use manage.py shell instead of python directly
stdin, stdout, stderr = c.exec_command("docker cp /tmp/create_admin.py hartbeat-backend:/tmp/create_admin.py && docker exec hartbeat-backend bash -c 'cd /app && cat /tmp/create_admin.py | python manage.py shell' 2>&1")
