...
# Create a habit material using habit_template
import io
...
print("Create Material Response:")
print(json.dumps(json.loads(resp2.read()), indent=2))
'''

sftp = c.open_sftp()
with sftp.open("/tmp/test_habit_material.py", "w") as f:
    f.write(script)
sftp.close()

stdin, stdout, stderr = c.exec_command("docker cp /tmp/test_habit_material.py hartbeat-backend:/tmp/test_habit_material.py && docker exec hartbeat-backend python /tmp/test_habit_material.py 2>&1")
